 ...
 */
public class ActiveGroupListAdapter extends LoadMoreListViewAdapter {
    ...
    public void subOnBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ...
        if (squareGroup.yesterdayGroupDailyStatistics != null)
            viewHolder.groupInfoText.setText("会长: " + squareGroup.getLeader().getNick_name() + " | " + "活跃度: " + (int) Math.floor(squareGroup.yesterdayGroupDailyStatistics.getActiveness() * 100) + "%");
        else
            viewHolder.groupInfoText.setText("会长: " + squareGroup.getLeader().getNick_name() + " | " + "活跃度: 0");

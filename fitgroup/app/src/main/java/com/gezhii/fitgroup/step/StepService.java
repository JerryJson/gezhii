package com.gezhii.fitgroup.step;
...
//        mUtils.initTTS();
        ...
        Log.i("darren-------", "steps:" + UserStepModel.getInstance().getUserTodayStep());
    ...
    public void reloadSettings() {
        ...
        if (mStepDetector != null) {
            mStepDetector.setSensitivity(
                    Float.valueOf(mSettings.getString("sensitivity", "10"))//设置灵敏度
            );
        }
...
//            Log.i("logger-----","isWrite:---"+isWrite);
            if (isWrite) {
                currentTime = System.currentTimeMillis();
                mStateEditor = mState.edit();
                // mStateEditor.putInt("steps", mSteps);
                com.xianrui.lite_common.litesuits.android.log.Log.i("darren ", "mSteps" + mSteps);
                UserStepModel.getInstance().updateTodayStep(mSteps);
                mStateEditor.putInt("pace", mPace);
                mStateEditor.putFloat("distance", mDistance);
                mStateEditor.putFloat("speed", mSpeed);

...
public class MyFollowerFragment extends BaseFragment {
    ...
    public static void start(Activity activity, int user_id) {
        ...
        mainActivity.showNext(MyFollowerFragment.class,params);
    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if ((int)getNewInstanceParams().get("user_id") == -1) {
            ...
            MobclickAgent.onEvent(getActivity(), "FansList", UmengEvents.getEventMap("click", "load", "from", "my"));
        } else {
            titleText.setText("TA的粉丝");
            MobclickAgent.onEvent(getActivity(), "FansList", UmengEvents.getEventMap("click", "load", "from", "others"));
            user_id = (int) getNewInstanceParams().get("user_id");
        }
        myFollowerListview.setLayoutManager(new LinearLayoutManager(getActivity()));
        API.getFollowedUsers(1, user_id, Config.loadPageCount, new APICallBack() {
            @Override
